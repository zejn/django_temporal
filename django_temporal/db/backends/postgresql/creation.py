...
IF EXISTS (
    SELECT * FROM %(referencing_table)s AS A
        WHERE next(A.%(referencing_validity_field)s) = TIMESTAMP WITH TIME ZONE ''9999-12-31 23:59:59.999999+0000''
        AND NOT EXISTS (
            SELECT * FROM %(referenced_table)s AS B
            WHERE A.%(referencing_field)s = B.%(referenced_field)s
            AND next(B.%(referenced_validity_field)s) = TIMESTAMP WITH TIME ZONE ''9999-12-31 23:59:59.999999+0000''
    )
) THEN
